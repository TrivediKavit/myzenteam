<template>
    <div class="overflow-hidden overflow-x-auto min-w-full align-middle sm:rounded">
        ...
        <form v-if="html_snippet" @submit.prevent="saveHtmlSnippet">
            <div class="mb-6">
...
onMounted(async () => {
    await getHtmlSnippet(props.id);
    loading.value = false;

    if(html_snippet.value.length == 0) {
        no_resource.value = true;
    }

    form.title = html_snippet.value.title;
    form.description = html_snippet.value.description;
    form.snippet = html_snippet.value.snippet;
});

const saveHtmlSnippet = async () => {
    submit_button.value = true;
    await updateHtmlSnippet(props.id, {
        "title": form.title,
