<template>
    <div v-if="admin" class="flex items-center justify-between flex-wrap mb-4">
        <div class="flex items-center flex-no-shrink mx-auto lg:mr-auto">
            <h4 class="font-medium leading-tight text-xl mt-2 mb-3">HTML Snippets</h4>
        </div>
        <div class="w-full text-center block flex-grow lg:flex lg:items-center lg:justify-end lg:w-auto">
            <div>
                <router-link :to="{ name: 'html-snippets.create' }" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 active:bg-green-900 focus:outline-none disabled:opacity-25 transition ease-in-out duration-150">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    Create New HTML Snippet
                </router-link>
            </div>
        </div>
    </div>
    <h4 v-else class="font-medium leading-tight text-xl mt-2 mb-3">HTML Snippets</h4>
    <table class="min-w-full divide-y divide-gray-200 table-fixed">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            <template v-if="loading && !no_resources">
                <tr class="">
                    <td colspan="4" class="py-4 px-6 text-md font-medium text-center text-gray-900 whitespace-nowrap">
            ...
            <template v-else-if="!loading && no_resources">
                <tr class="">
                    <td colspan="4" class="py-4 px-6 text-md font-medium text-center text-gray-900 whitespace-nowrap">
                        No HTML Snippets Found
                    </td>
                </tr>
            </template>
            <HtmlSnippetsTableRow v-for="item in html_snippets" :item="item" :admin="admin" :key="item.id" />
        </tbody>
    </table>
</template>
