<template>
    <div v-if="admin" class="flex items-center justify-between flex-wrap mb-4">
        <div class="flex items-center flex-no-shrink mx-auto lg:mr-auto">
            <h4 class="font-medium leading-tight text-xl mt-2 mb-3">Links</h4>
        </div>
        <div class="w-full text-center block flex-grow lg:flex lg:items-center lg:justify-end lg:w-auto">
            <div>
                <router-link :to="{ name: 'links.create' }" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 active:bg-green-900 focus:outline-none disabled:opacity-25 transition ease-in-out duration-150">
    ...
    <h4 v-else class="font-medium leading-tight text-xl mt-2 mb-3">Links</h4>
    <table class="min-w-full divide-y divide-gray-200 table-fixed">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <LinksTableRow v-for="item in links" :item="item" :admin="admin" :key="item.id"/>
        </tbody>
    </table>
</template>
